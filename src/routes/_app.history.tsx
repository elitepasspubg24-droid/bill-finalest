...
function HistoryPage() {
  ...
  const carryForwardHistory = useMemo(() => {
    ...
    // Generate every day from the first record until today
    ...
    // Fill every day in the interval
    return days.map(day => {
      const dStr = format(day, "yyyy-MM-dd");
      const changes = historyByDay.get(dStr);
      
      // If there are changes today, update lastKnown. Otherwise, lastKnown keeps previous value.
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">Showing rates for every day. Gaps are filled with the last known price.</p>
      {carryForwardHistory.map((day) => (
        <Card key={day.date}>
          <CardHeader className="py-2 px-4 border-b bg-muted/20">
            <CardTitle className="text-xs font-bold">{format(new Date(day.date), "dd MMM yyyy")}</CardTitle>
          </CardHeader>
          <CardContent className="p-3 grid grid-cols-2 md:grid-cols-6 gap-2">
            {factories.data?.map(f => (
              <div key={f.id} className="rounded border p-2 bg-background shadow-sm">
                <div className="text-[9px] text-muted-foreground uppercase font-bold">{f.name}</div>
                <div className="font-mono text-sm font-bold">{day.rates.get(f.id) ?? "—"}</div>
              </div>
            ))}
          </CardContent>
        </Card>
